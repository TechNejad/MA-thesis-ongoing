<div align="center">

# Towards an Ontology of Normative Role Design for LLM interactions in Multi-Agent Simulations

### [Mohsen Hassan Nejad](https://mohsen-hassan-nejad.netlify.app/)

**Erasmus Mundus Joint Master's Scholarship in Artificial Intelligence for Sustainable Societies (AISS)**  
**Ongoing thesis (2025-2026)**

<p>
  <a href="https://technejad.github.io/MA-thesis-ongoing/"><img alt="Research Homepage" src="https://img.shields.io/badge/Research%20Homepage-253246?style=for-the-badge"></a>
  <a href="https://technejad.github.io/MA-thesis-ongoing/one-pager/"><img alt="One-Pager Summary" src="https://img.shields.io/badge/One--Pager%20Summary-253246?style=for-the-badge"></a>
  <a href="https://technejad.github.io/MA-thesis-ongoing/conference.html"><img alt="Conference Page" src="https://img.shields.io/badge/Conference%20Page-253246?style=for-the-badge"></a>
  <a href="https://mohsen-hn.com/assets/Ontology_of_normative_design.pdf"><img alt="Conference PDF" src="https://img.shields.io/badge/Conference%20PDF-1a73e8?style=for-the-badge"></a>
</p>

</div>

## Abstract

Ongoing master's thesis exploring how normative roles for large language models are defined and operationalized within multi-agent simulations. As LLMs evolve into autonomous agents, their roles are encoded through system prompts, yet their development and evaluation remain ad hoc and unsystematic. This project aims to develop a preliminary ontology of normative role design by mapping how roles are operationalized and how they shape outcomes.

## Research Approach

- **Systematic Literature Review**: identify and analyse multi-agent LLM simulation studies and their code.
- **Five-Layer Coding**: code each study across study context, simulation setup, normative design, outcomes, and evidence.
- **Ontology Construction**: formalise findings as knowledge triples linking role types, prompting methods, ethical framings, and outcomes.

## Repository Structure

```text
MA-thesis-ongoing/
├── index.html          # research homepage
├── style.css           # shared stylesheet
├── conference.html     # disclaimer page before conference PDF
└── one-pager/
    └── index.html      # thesis one-pager
```

## Notes

The conference paper page includes a disclaimer because the paper reflects only a fraction of the broader thesis. The conference paper is based on two initial simulation studies, while the thesis database includes 40 studies. Full thesis findings are expected by the end of April.
